"""Implementation of OCR Inference."""
...
from ocr.builder.openvino_inference import InferenceRecOV, InferenceDetOV

from ocr.modules.common import BadImageException
from ocr.builder.onnx_inference import InferenceRecONNX, InferenceDetONNX
...
class InferenceOCR:
    ...
    def _create_modules(self) -> None:
        ...
        Raises:
            KeyError: Raises when incorrect speed type param pass.
        """
        self._load_configs()
    ...
    def _load_configs(self) -> None:
        ...
        with open("configs/inference.yaml", "r") as file:
            ...
            self.device = "cuda" if self.config["speed_up"] == "cuda" else "cpu"
    ...
    def _check_image(self, image: np.ndarray) -> None:
        ...
        Raises:
            BadImageException: Raised when incorrect image type was loaded to inference.
                Should be RGB np.uint8.
        """
        if not isinstance(image, np.ndarray):
            raise BadImageException(
                "Incorrect image type!" "Expected np.ndarray, " f"got {type(image)}"
            )
    ...
    def _create_crops(
        self, prediction: List[np.ndarray], image: np.ndarray
    ) -> Dict[int, np.array]:
        ...
        Args:
            prediction (list of np.ndarray): OCR prediction.
            image (np.ndarray): On the iamge make crops.

        Returns:
            dict of str: np.ndarray: Dict with crops.
        """
        crops = dict()
        for idx, pred in enumerate(prediction):
            ...
            if (
                abs(x_2 - x_1) < self.config["min_crop_size"][0]
                or abs(y_2 - y_1) < self.config["min_crop_size"][1]
            ):
                continue
    ...
    def _visualize_prediction(
        self, image: np.ndarray, det_prediction: List[np.ndarray], text: Dict[int, str]
    ) -> np.ndarray:
        """Show detected words on the input image.

        Args:
            image (np.ndarray): Visualize on the image.
            det_prediction (list of np.ndarray): OCR Detection prediction.
            text (dict of int: str): OCR Recognition prediction.

        Returns:
            np.ndarray: Image with visualized prediction.
        """
        # Show text.
        ...
        image = self.det_model.visualize(
            image=image, prediction=np.copy(det_prediction)
        )
        return image
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        Args:
            image (np.ndarray): On the iamge make predictions.

        Returns:
            np.ndarray: OCR prediction.
        """
        self._check_image(image)

        det_prediction = self.det_model.predict(image)
        crops = self._create_crops(det_prediction, image)
        text = {key: self.rec_model.predict(val) for key, val in crops.items()}
