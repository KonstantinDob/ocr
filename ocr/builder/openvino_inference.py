...
class InferenceRecOV(BaseInferenceOCRRec):
    ...
    def _create_modules(self):
        ...
        network = ie_core_handler.read_network(model=xml_path,
                                               weights=bin_path)
        self.model = ie_core_handler.load_network(network,
                                                  device_name='CPU',
                                                  num_requests=1)
    ...
    def predict(self, image: np.ndarray) -> str:
        ...
        image = np.transpose(np.float32(image)[np.newaxis],
                             (0, 3, 1, 2))
        ...
        prediction = inference_request.output_blobs[
            output_blob_name].buffer
...
class InferenceDetOV(BaseInferenceOCRDet):
    ...
    def predict(self, image: np.ndarray) -> List[np.ndarray]:
        ...
        inference_request.infer()
        output_blob_name = next(iter(inference_request.output_blobs))
        prediction_mask = inference_request.output_blobs[
            output_blob_name].buffer
